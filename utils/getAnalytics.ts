...
const getIndexOfXOption = (
  initialValue: number,
  arrayLength: number,
  queryDateValue: number
) => {
  const arr: number[] = [];
...
const getXOptionsForGraphic = (
  ...
  if (years === 1) {
    ...
    const arr: number[] = [];
    for (
      let i = getMonth(startDate);
      i < getMonth(startDate) + arrLength + 1;
      i++
    ) {
    ...
    if (
      getYear(queryDate) === getYear(startDate) ||
...
export const getAnalyticsForGraphic = (
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    const xOption = getXOptionsForGraphic(
      queries[i].date,
      months,
      years,
      startDate,
...
export const getAnalyticsForPieChart = (
  initiativeValues,
  statisticItems,
...
const convertStatus = (status: string) => {
  if (statusOptionsProcess.indexOf(status) !== -1) return "В процессе";
  if (status === "Выполнена") return "Выполнены";
  else return "Отклонены";
};
...
export const filterAnalytics = (
  ...
  return queries.filter(
    (query) => new Date(query.date) <= end && new Date(query.date) >= start
  );
};
